...
export default function WishlistCard({ item, onRemove }) {
  return (
    <div className="bg-card rounded-xl border border-divider overflow-hidden group">
      ...
      <div className="p-4">
        <Link to={`/renter/rent_items/${item.id || item._id}`}>
        ...
        <p className="text-sm text-text-primary mb-2 flex items-center gap-0.5">
          <div className='flex items-center gap-1.5'>
            <Star className='size-4 text-bright fill-bright' />
            {typeof item.rating === "object" ? (item.rating?.average ?? 0) : (item.rating ?? 0)}
          </div>
          <span className="text-xs text-text-secondary">({typeof item.reviews === "object" ? (item.reviews?.count ?? 0) : (item.reviews ?? 0)} reviews)</span>
        </p>

        <div className="flex justify-between items-center mt-4">
          <div>
            <p className="font-bold text-lg text-bright">
              ₹{typeof item.pricePerDay === "object" ? item.pricePerDay?.daily : (item.pricePerDay ?? (typeof item.price === "object" ? item.price?.daily : item.price) ?? 0)}
